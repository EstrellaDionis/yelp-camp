...
module.exports.showCampground = async (req, res) => {
  const campground = await Campground.findById(req.params.id)
    .populate({
      ...
      populate: {
        path: "author" //then on each of the reviews, populate their authors
      }
    })
    .populate("author"); //populating the campgrounds author
  if (!campground) {
...
module.exports.updateCampground = async (req, res) => {
  ...
  //.push because we want to add on, not replace
  ...
  if (req.body.deleteImages) {
    ...
    // pull from images, the filename, that is in, req.body.deleteImages
